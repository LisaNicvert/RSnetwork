...
simu_file <- file.path(outputs_path, "simres.rds")
```
...
param_sdtol <- data.frame(ninter = ninter_def,
                          ...
                          delta = delta_def,
                          exp = "sd_tol")
param_delta <- data.frame(ninter = ninter_def,
                          nconsumer = nconsumer_def,
                          nresource = nresource_def,
                          abund_distri = abund_distri_def,
                          mean_tol = mean_tol_def,
...
legend_df$legend <- c(expression("Number of interactions ("*italic(n)[inter]*")"),
                      expression("Species abundance distribution ("*bold(n^r)*","*bold(n^c)*")"),
                      expression("Mean consumers niche breadth ("*italic(mu)[breadth]*")"),
                      expression("Standard deviation of consumers niche breadth ("*sigma[breadth]*")"),
                      expression("Trait matching strength ("*delta*")"))
```

## Simulations

`perform_simu` is `r perform_simu` so the simulations will be `r ifelse(perform_simu, "performed", paste("loaded from the file", paste(basename(simu_file), collapse = ", ")))`.
...
if (perform_simu) {
  ...
  # Save result
  saveRDS(simres, file = simu_file)
  
  message(paste0("Output of system.time:"))
  print(rtime)
...
We choose one of the simulation settings to visualize data.
...
dfmean <- simres$cor$realized$mean_cor
dfsd <- simres$cor$realized$sd_cor
...
for(exp_plot in unique(dfmean$exp)) { # experiments
  ...
  boxplots <- g1 / g2 + 
    ...
    scale_x_continuous(breaks = c("Axis 1" = 1, "Axis 2" = 2)) &
    theme_linedraw() &
    ylab("Correlation (absolute value)") &
    theme(legend.position = 'bottom',
          axis.title.x = element_blank(),
          axis.text = element_text(size = txtsize*0.9),
          axis.title.y = element_text(size = txtsize),
          legend.title = element_text(size = txtsize),
          legend.text = element_text(hjust = 0.5, 
                                     size = txtsize*0.9),
          plot.title = element_text(size = txtsize*1.1),
          ...
          strip.text = element_text(color = "black", size = txtsize))
  ...
  boxplots <- boxplots / guide_area() / 
    ...
    plot_layout(heights = c(3, 3, .8, 1.5))
...
for (n in names(boxplots_list)) {
  # ggsave doesn't work well with formulas in axes
  # png(file.path(figures_path, paste0("boxplots_", n, ".png")),
  #     width = 15, height = 18,
  #     units = "cm",
  #     res = 600)
...
We check how eigenvalues change for different values of $\delta$.
...
eig_lab <- eigval_df_summary |> 
  group_by(eig_name) |> 
  summarize(y = max(median))
eig_lab <- eig_lab |> 
  mutate(name = paste0("sqrt(lambda[", str_replace(eig_name, "l", ""), "])")) |> 
  arrange(eig_name) |> 
  mutate(vjust = seq(-0.2, 1.3, length.out = nrow(eig_lab)))

# ggsave doesn't work with formulas in axes
# png(file.path(figures_path,
#               paste0("lambda_evolution_delta.png")),
#     width = 15, height = 18,
#     units = "cm",
#     res = 600)
ggplot(eigval_df_summary, 
       aes(x = delta, group = eig_name)) +
  ...
  scale_x_continuous(breaks = seq(0, 1, by = 0.2),
                     limits = c(0, 1.07)) +
  geom_text(data =  eig_lab, aes(x = 1.03, y = y, 
                                 label = name, 
                                 col = eig_name,
                                 vjust = vjust), 
            size = 3.4,
            ...
            parse = TRUE) +
  theme_linedraw() +
  xlab(expression("Trait matching strength ("*delta*")")) +
...
# Compute intermediate sum of eigenvalues (when delta varies)
chisq_summary <- eigval |> 
  filter(exp == exp_plot) |> 
...
chisq_summary <- chisq_summary |>
  group_by(id) |>
  mutate(p_adjusted = p.adjust(pval,
                               method = "holm", n = nrep)) |>
...
ggplot(chisq_summary, aes(x = delta)) +
...
for (i in 1:nrow(id_exp)) {
  id <- id_exp[i, "id"]
  delta <- id_exp[i, "delta"]
...
# Get fundamental niches
dfsd_fu <- simres$cor$fundamental$sd_cor
dfmean_fu <- simres$cor$fundamental$mean_cor
...
```{r, fig.height=8, fig.width=6}
for (n in names(boxplots_list)) {
  # png(file.path(figures_path,paste0("re_fu_boxplots_",
  #                                   n, ".png")),
  #     width = 15, height = 18,
  #     units = "cm",
  #     res = 600)
