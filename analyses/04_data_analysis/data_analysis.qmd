...
plot_matrix(interactions, 
  ...
  theme(legend.key.width = unit(0.1, 'cm'),
        ...
        axis.line.y = element_line(colour = params$colli, 
                                   linewidth = 1),
        axis.text = element_text(size = txtsize*0.4),
...
plot_eig(ca$eig, showrank = TRUE) +
  ylab("Eigenvalues") +
  theme(axis.title = element_text(size = 15),
        axis.text = element_text(size = 10))
# ggsave(file.path(figures_path, "eig.png"),
#        width = 7, height = 4.5, dpi = 300)

plot_eig(sqrt(ca$eig), showrank = TRUE) + 
  ylab("Square root of eigenvalues") +
  theme(axis.title = element_text(size = 15),
        axis.text = element_text(size = 10))
ggsave(file.path(figures_path, "sqeig.png"),
       width = 7, height = 4.5, dpi = 300)

plot_eig(cumsum(ca$eig)/sum(ca$eig), showrank = TRUE) +
  ylab("Relative cumulative sum of eigenvalues") +
  theme(axis.title = element_text(size = 15),
        axis.text = element_text(size = 10))
ggsave(file.path(figures_path, "cumsumeig.png"),
       width = 7, height = 4.5, dpi = 300)
```
...
for(a in ax){
  ...
  lmplots[[a]] <- plot_lm_mean_sd(msd_merge, 
                                  ...
                                  lab = lab_df,
                                  seed = 123,
                                  ylab = paste0("Niche breadth\n(standard deviation on axis ", a, ")"), 
                                  xlab = paste0("Latent trait (mean position on axis ", a, ")"), 
                                  facet = "type")
...
plot_corcircle(cor = corli,
               ...
               labsize = 5,
               eig = ca$eig) +
  theme(axis.title = element_text(size = txtsize),
...
Then, we plot the relationship between CA coordinates and the residuals of these models.
